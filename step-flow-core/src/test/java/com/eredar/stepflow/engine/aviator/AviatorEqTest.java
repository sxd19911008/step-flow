...
/**
 * Aviator 等于运算单元测试，对应 {@code OperatorType.EQ}（即 {@code ==} 号）。
 */
@DisplayName("Aviator 等于测试")
public class AviatorEqTest {
    ...
    static Stream<Arguments> testStringProvider() {
        return Stream.of(
                // 左操作数为 String 时与数值比较：等于返回 false
                Arguments.of(
                        "a == b",
                        HashMapBuilder.<String, Object>builder().put("a", "12").put("b", 10L).build(),
                        false
    ...
    static Stream<Arguments> testBooleanProvider() {
        return Stream.of(
                ...
                Arguments.of(
                        "a == b",
                        HashMapBuilder.<String, Object>builder().put("a", false).put("b", false).build(),
                        true
                )
        );
    }
